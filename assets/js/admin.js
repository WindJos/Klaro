...
async function seConnecter() {
  ...
  if (!window.KlaroDB || !window.KlaroDB.db) {
    afficherErreurGate(
      'Connexion impossible. Ouvre le site depuis Netlify, pas en local.'
    );
  ...
  /* ── Appel Supabase Auth ── */
  const { data, error } = await window.KlaroDB.db.auth.signInWithPassword({
    email:    email,
    password: motDePasse,
  });
...
async function verifierSessionExistante() {
  ...
  const { data: { session } } = await window.KlaroDB.db.auth.getSession();
